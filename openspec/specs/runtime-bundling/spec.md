# runtime-bundling Specification

## Purpose

Ship the container runtime and k3c-owned helper binaries embedded in the k3c
binary, and extract them to the host cache so every command resolves the exact
payload the running binary carries. This capability owns the embedded-payload
extraction and its cache identity, plus the release-CI guard that verifies the
payload is complete. Getting the extraction identity right is what keeps an
upgrade from silently running stale helper binaries.
